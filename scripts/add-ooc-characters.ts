/**
 * Adds five showcase characters inspired by ooc.ai archetypes.
 * Idempotent: skips any character whose name already exists.
 * Run with: npx tsx scripts/add-ooc-characters.ts
 */
import { db } from "../src/server/db";
import { createCharacter } from "../src/server/services/character";

const CREATOR = (() => {
  const row = db
    .prepare("SELECT id FROM users WHERE email = 'demo@aetheria.dev' LIMIT 1")
    .get() as any;
  if (!row) throw new Error("demo user not found — run the seed first");
...
for (const c of FIVE) {
  const existing = db
    .prepare("SELECT id FROM characters WHERE name = ? LIMIT 1")
    .get(c.name) as any;
  if (existing) {
    console.log(`- ${c.name}: already exists, skipped`);
    continue;
  }
  const created = createCharacter(CREATOR, { ...c, isPublic: true, allowRemix: true });
